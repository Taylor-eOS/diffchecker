...
def normalize_line(line):
    ...
    if result[:1].isdigit():
        idx = result.find(" ")
        if idx != -1 and result[:idx].isdigit():
            result = result[idx + 1:]
    out = []
    i = 0
    n = len(result)
    while i < n:
        c = result[i]
        if c == "0" and i + 1 < n and result[i + 1] in ("x", "X"):
            j = i + 2
            while j < n and result[j] in "0123456789abcdefABCDEF":
                j += 1
            out.append("<HEX>")
            i = j
            continue
        if c.isdigit() or (c == "-" and i + 1 < n and result[i + 1].isdigit()):
            start = i
            i += 1
